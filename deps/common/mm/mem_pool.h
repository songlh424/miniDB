...
{
public:
  using item_unique_ptr = unique_ptr<void, function<void(void *const)>>;  // 带删除器
...
public:
  MemPoolItem(const char *tag) : name(tag)
  {
    ...
    pthread_mutexattr_t mutexatr;
    pthread_mutexattr_init(&mutexatr);
    pthread_mutexattr_settype(&mutexatr, PTHREAD_MUTEX_RECURSIVE);  // 递归锁，可重复上锁
   ...
   */
  void           *alloc();
  item_unique_ptr alloc_unique_ptr(); // 带删除器
...
protected:
  pthread_mutex_t mutex;     // mutex for memory pool
  string          name;      // name of memory pool 
  bool            dynamic;    // whether the memory pool is dynamic
  int             size;      // size of item nums in memory pool
  int             item_size;  // size of item
  int             item_num_per_pool;    // how many items per pool
  
  // 先分配大块内存，记录到pools中，再将大块内存分割成小块内存，记录到frees中
  list<void *> pools; // memory pool
  set<void *>  used;   // used items
  list<void *> frees; // free items
};
