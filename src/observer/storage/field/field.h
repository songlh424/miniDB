...
public:
  ...
  void set_int(Record &record, int value);  // 设置字段值到record中
  int  get_int(const Record &record);       // 从record中获取字段值

  const char *get_data(const Record &record);

private:
  const Table     *table_ = nullptr;   // 表
  const FieldMeta *field_ = nullptr;   // 字段元数据
};
