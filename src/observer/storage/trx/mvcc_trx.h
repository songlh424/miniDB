...
private:
  ...
  atomic<int32_t> current_trx_id_{0}; // 当前事务号，每次创建新事务时，需要递增

  common::Mutex lock_; // 互斥锁，用于保护事务列表
  vector<Trx *> trxes_;       // 存储所有事务
};
...
private:
  ...
  MvccTrxKit       &trx_kit_;             // 所属的事务管理器
  MvccTrxLogHandler log_handler_;         // 日志处理器
  int32_t           trx_id_     = -1;
  bool              started_    = false;   // 是否已经启动
  bool              recovering_ = false;   // 是否正在恢复
  OperationSet      operations_;           // 记录当前事务所有操作
};
