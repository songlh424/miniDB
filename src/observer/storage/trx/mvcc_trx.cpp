...
RC MvccTrx::insert_record(Table *table, Record &record)
{
  Field begin_field;
  Field end_field;
  trx_fields(table, begin_field, end_field); // 获取事务字段，拿到begin_field和end_field字段地址

  begin_field.set_int(record, -trx_id_); // 设置事务开始字段
  end_field.set_int(record, trx_kit_.max_trx_id()); // 设置事务结束字段
...
RC MvccTrx::visit_record(Table *table, Record &record, ReadWriteMode mode)
{
  ...
  if (begin_xid > 0 && end_xid > 0) {
    // begin_xid end_xid大于0说明是已经提交的数据
    if (trx_id_ >= begin_xid && trx_id_ <= end_xid) { // 当前事务在记录的事务范围内
      rc = RC::SUCCESS;
    } else {
...
RC MvccTrx::commit_with_trx_id(int32_t commit_xid)
{
  ...
  for (const Operation &operation : operations_) {
    switch (operation.type()) {
      ...
      case Operation::Type::DELETE: {       // 删除操作
        Table *table = operation.table();
        RID    rid(operation.page_num(), operation.slot_num());
