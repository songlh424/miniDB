...
RC RowRecordPageHandler::insert_record(const char *data, RID *rid)
{
  ...
  RC rc = log_handler_.insert_record(frame_, RID(get_page_num(), index), data);   // 操作先写到日志中
  if (OB_FAIL(rc)) {
  ...
  char *record_data = get_record_data(index);       // 再写到内存的页帧中
...
{
  if (disk_buffer_pool_ != nullptr) {     // 防止多次调用init函数
