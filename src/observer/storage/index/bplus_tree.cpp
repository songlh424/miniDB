...
bool IndexNodeHandler::is_safe(BplusTreeOperationType op, bool is_root_node)
{
  switch (op) {
    ...
    case BplusTreeOperationType::INSERT: {
      return size() < max_size();           //最大值应该根据节点大小和一条记录的大小来计算
    } break;
            ...
            int leaf_max_size /* = -1 */)
{
  ...
  memcpy(&file_header_, pdata, sizeof(file_header_)); // 这里是拷贝一份到内存中，方便后续使用
  header_dirty_ = false;
    ...
    BplusTreeMiniTransaction &mtr, BplusTreeOperationType op, PageNum page_num, bool is_root_node, Frame *&frame)
{
  ...
  mtr.latch_memo().latch(frame, latch_type);  // 对页面帧进行锁定
...
RC BplusTreeHandler::insert_entry(const char *user_key, const RID *rid)
{
  ...
  if (is_empty()) {   // 空树
    root_lock_.lock();
    if (is_empty()) {   // 双重检查
      rc = create_new_tree(mtr, key, rid);  // 创建树
      root_lock_.unlock();
      return rc;
    }
    root_lock_.unlock();
